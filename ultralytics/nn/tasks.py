...
from ultralytics.nn.modules.MB import MB_TaylorFormer
from ultralytics.nn.modules.block import C3k2
# 新增修改（手动导入自定义模块，确保框架能找到
from ultralytics.nn.modules.block import DAttentionBaseline
import contextlib
import pickle
import re
import types
...
C3K2_CLASS = (C3k2_Faster, C3k2_ODConv, C3k2_Faster_EMA, C3k2_DBB, C3k2_DeepDBB, C3k2_WDBB, C3k2_CloAtt, C3k2_SCConv, C3k2_ScConv, C3k2_EMSC, C3k2_EMSCP, C3k2_KW, C3k2_DySnakeConv, C3k2_DCNv2, C3k2_DCNv3,
              ...
              C3k2_ConverseB, C3k2_Converse, C3k2_GCConv, C3k2_CFBlock, C3k2_FMABlock, C3k2_LWGA
              )
...
C2PSA_CLASS = (C2PSA, C2BRA, C2CGA, C2DA, C2DPB, C2Pola, C2TSSA, C2ASSA, C2PSA_DYT, C2TSSA_DYT, C2Pola_DYT, C2PSA_FMFFN, C2PSA_CGLU, C2PSA_SEFN, C2PSA_Mona, C2PSA_SEFFN, C2TSSA_DYT_Mona, C2TSSA_DYT_Mona_SEFN, C2TSSA_DYT_Mona_SEFFN, C2PSA_EDFFN,
               C2TSSA_DYT_Mona_EDFFN, C2MSLA, C2PSA_EPGO, C2PSA_DML)
...
def get_feature_channels(feature_info):
    """
    兼容提取 feature_info 中的通道数，支持多种格式
    Args:
        feature_info: 模型的 feature_info 属性
    Returns:
        list: 输出通道列表
    """
    # 情况 1：有 channels() 方法（原有 timm 模型逻辑）
    if hasattr(feature_info, "channels") and callable(getattr(feature_info, "channels")):
        return feature_info.channels()
    # 情况 2：有 channels 属性（列表/元组格式）
    elif hasattr(feature_info, "channels") and isinstance(feature_info.channels, (list, tuple)):
        return feature_info.channels
    # 情况 3：有 num_chs 属性（自定义模型，如 MB_TaylorFormer）
    elif hasattr(feature_info, "num_chs"):
        return [feature_info.num_chs]
    # 情况 4：兜底返回默认值（避免报错）
    else:
        LOGGER.warning("WARNING ⚠️ 无法提取 feature_info 通道数，使用默认值")
        return [64]  # 默认通道数，可根据你的模型调整
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            C3k2_WTConv, C3k2_MSCB1, C3k2_MSCB2, DynamicConvModule, ODConv, DAttentionBaseline, StarNet, PENet, MB_TaylorFormer,
            RepStem
        ) + C3K2_CLASS + A2C2F_CLASS + C2PSA_CLASS):
            #if args[0] == 'head_channel':
                #args[0] = d[args[0]]
            if args and args[0] == 'head_channel':  # 先判断args非空，再访问args[0]
                args[0] = d[args[0]]
            #c1, c2 = ch[f], args[0]
            ...
            c1 = ch[f]
            if not args:  # args为空时，根据模块类型设置默认c2
                if m in (Conv, DWConv, GhostConv):
                    c2 = c1 * 2  # Conv类默认输出通道为输入的2倍
                elif m in (C2f, C3k2, C3, C2fPSA):
                    c2 = c1  # C2f/C3k2默认输出通道等于输入
                elif m in (Classify,):
                    c2 = d.get('nc', 80)  # Classify默认输出通道为类别数
                elif m in (SPP, SPPF):
                    c2 = c1  # SPP/SPPF默认输出通道等于输入
                else:
                    c2 = c1  # 其他模块默认输出通道等于输入
                args = [c2]  # 初始化args为默认值
            else:
                c2 = args[0]  # args非空时正常取值
            
            # 第三步：原有逻辑（通道数缩放）
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            
            # 第四步：C2fAttn特殊处理（增加args非空判断）
            if m is C2fAttn:
                if len(args) > 1:
                    args[1] = make_divisible(min(args[1], max_channels // 2) * width, 8)  # embed channels
                else:
                    args.append(make_divisible(min(c1//2, max_channels // 2) * width, 8))
                ...
                if len(args) > 2:
                    ...
                    args.append(int(max(round(min(8, max_channels // 2 // 32)) * width, 1)))
            ...
            if m in ((
                ...
                MANet_FasterBlock, MANet_FasterCGLU, MANet_Star, CSP_MSCB_SC, MANet_GCConv, C3k2_WTConv, C3k2_MSCB1, C3k2_MSCB2, DynamicConvModule,
                ODConv, DAttentionBaseline, StarNet, PENet, MB_TaylorFormer
            ) + C3K2_CLASS + A2C2F_CLASS + C2PSA_CLASS):
        ...
        elif m in {AIFI, AIFI_RepBN, DTAB, ETB, FDT, MB_TaylorFormer}:
            args = [ch[f], *args]
            c2 = args[0]
        ...
        #     args = [c2, *args]
            ...
            #if args[0] == 'head_channel':
                #args[0] = d[args[0]]
            if args and args[0] == 'head_channel':  # 增加非空判断
                args[0] = d[args[0]]
            c1 = [ch[x] for x in f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
